...
class _EditAddressScreenState extends ConsumerState<EditAddressScreen> {
  ...
  Future<void> _loadCurrentAppData() async {
    ...
    try {
      // Load current user mobile number from auth provider with retry logic
  ...
  Future<void> _loadUserMobileWithRetry() async {
    ...
    try {
      ...
      if (userProfile != null && userProfile.mobile.isNotEmpty) {
        ...
        logger.error('Could not load user mobile number after all attempts, using address mobile');
        _contactNumberController = TextEditingController(text: _address.mobileNumber);
      }
    } catch (e) {
      logger.error('Error accessing auth manager directly, using address mobile: $e');
      _contactNumberController = TextEditingController(text: _address.mobileNumber);
    }
  }
